...
- **`Di.ApplicationContext`:** The runtime owner for module registries, global provider publication, Custom Metadata loading/cache state, and test metadata mocks. Static `Di` methods delegate to a default application context, while explicit contexts can be created with `Di.createContext()` to isolate graphs in the same transaction.
